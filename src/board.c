...
int load_pacman(board_t *board, const char *directory, const char *filename, int points) {
    ...
    while (read_line(fd, line, sizeof(line)) > 0) {
        ...
        if (strncmp(ptr, "PASSO", 5) == 0) {
            ...
            char c = ptr[0];
            if (c == 'A' || c == 'D' || c == 'W' || c == 'S' || c == 'R' || c == 'C' || c == 'Q' || c == 'G') {
                board->pacmans[0].moves[n_moves].command = c;
...
int load_ghost(board_t *board, int ghost_index, const char *directory, const char *filename) {
    ...
    while (read_line(fd, line, sizeof(line)) > 0) {
        ...
        if (strncmp(ptr, "PASSO", 5) == 0) {
            ...
            char c = ptr[0];
            if (c == 'A' || c == 'D' || c == 'W' || c == 'S' || c == 'R' || c == 'C') {
                board->ghosts[ghost_index].moves[n_moves].command = c;
...
static void start_board(board_t *board, int n_ghosts, const char *level_name) {
    ...
    strncpy(board->level_name, level_name, sizeof(board->level_name) - 1);
}

// preenche o mapa do board
...
// inicializa o pacman manualmente
...
int load_level(board_t *board, const char *directory, int level_index, int points) {
    ...
    int n_levels;
    char **level_files = sorted_levels(directory, &n_levels); // ordena os níveis 
    ...
    read_level(fd, board, pacman_file, ghost_files, &n_ghosts, &map_lines, &map_height);
    close(fd);
    
    start_board(board, n_ghosts, level_files[level_index]);
    
    build_board(board, map_lines, map_height);
    ...
    if (strlen(pacman_file) > 0) {
        ...
        load_pacman_manual(board, points);
    }
...
void* pacman_thread(void* arg) {
    ...
    while (board->game_running && pac->alive) {
        ...
        command_t* c = &pac->moves[pac->current_move % pac->n_moves];
        ...
        if (c->command == 'G') {
            board->portal_reached = 2;  // 2 indica pedido de backup
            pac->current_move++;
            pthread_rwlock_unlock(&board->board_lock);
        ...
        int result = move_pacman(board, pacman_index, c);
...
void* ghost_thread(void* arg) {
    ...
    while (board->game_running) {
        ...
        move_ghost(board, ghost_index, c);
...
void stop_character_threads(board_t* board) {
    ...
    // espera pelas threads dos pacmans
    for (int i = 0; i < board->n_pacmans; i++) {
        if (board->pacmans[i].n_moves > 0) {
    ...
    // espera pelas threads dos fantasmas
    for (int i = 0; i < board->n_ghosts; i++) {
        pthread_join(board->ghosts[i].thread, NULL);
    }
}

// finaliza o rwlock do board
